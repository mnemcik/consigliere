...
This file provides guidance to Claude Code when working on the grimoire plugin itself.
...
skills/                           # Slash command definitions
  ...
  init/SKILL.md                   # /grimoire-init — workspace bootstrapper
...
- Templates in `templates/workspace/CLAUDE.md` use sentinel comments (`<!-- grimoire:section:start=X -->`) to delimit framework vs user sections
- Version in `.claude-plugin/plugin.json` must match version in `templates/workspace/.grimoire.json`
- Content templates are generic — no user-specific content

## Adding a New Skill

1. Create `skills/{skill-name}/SKILL.md` with YAML frontmatter
2. Include a guard clause that checks for `.grimoire.json` at workspace root
3. Update `README.md` with the new skill's documentation
4. Bump version in `.claude-plugin/plugin.json` and `templates/workspace/.grimoire.json`

## Release Process

1. Bump version in `.claude-plugin/plugin.json` and `templates/workspace/.grimoire.json`

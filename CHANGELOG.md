...
- **`cg active`, `cg tags`, `cg colors check` — read-only workspace scanners.** Cross-platform Go ports of `active-projects.sh`, `area-tags.sh`, and `colors-check.sh` (no `jq`/`awk`/`stat`). `cg active [--slugs|--json]` lists projects with a live session, applying the liveness rule (dirty within `session.dirtyWindowMin`, clean within `session.activeWindowMin`). `cg tags` counts the free-form area tags from each area's `- **Tags:**` Meta line, most-used first. `cg colors check` reports badge-color assignments, items without a color, and duplicate colors across `projects/*/README.md` + `areas/*.md`, exiting non-zero when duplicates exist so it can gate a pre-commit hook. Output and exit codes verified against the shell originals on the reference workspace.

- **`cg session start-gate` & `cg session statusline` — the session text/badge hook bodies.** `start-gate` (UserPromptSubmit) emits the session-start reminder, suppressing it for slash commands and quick/throwaway prompts, appending a main-worktree warning when the session shares the main git index, and best-effort pruning stale badge files. Its wording is **config-driven** (gap-analysis DEC-001): a workspace points `session.gateTemplate` at a file it owns (tokens `{{session_id}}`, `{{badge_file}}`, `{{worktree_warning}}`), otherwise a framework-neutral default is used — so workspace-specific rule text never lives in the binary. `statusline` renders a colored `[area/project]` badge (format via `session.badgeFormat`) with a trailing red ● when the session is dirty, resolving the color from the project/area `**Color:**` field and falling back to a deterministic hash, and optionally wrapping an upstream status line (`session.statuslineUpstream`). Cross-platform Go (no `jq`/`awk`/`cksum`); the workspace-specific caveman badge is dropped (re-addable via a wrapper).
- **`cg session mark-dirty` & `cg session pull-latest` — first two Claude Code hook bodies.** A new `internal/session` package + `cg session` command group port the workspace's shell hooks to cross-platform Go (no `jq`/`stat`/`timeout`/`date`). `mark-dirty` (PostToolUse) flips `"dirty": true` in the per-session badge file `.claude/session-context/<id>.json`, preserving any other fields and ignoring writes to the badge file itself; it is a no-op when the file doesn't exist (tracking is opt-in). `pull-latest` (SessionStart) fast-forwards the main worktree's landing branch from origin — skipping unless that root is on the landing branch with a clean tree — and reports fetch failures, successful pulls, and divergence via a `{"systemMessage": …}` object (a bounded fetch timeout keeps it under the hook budget). Both are designed never to fail the session. Hooks ship as thin bash wrappers calling these (a later PR).

- **`cg worktree remove <slug> [--force]` — tear down a session worktree.** Ports `remove-session-worktree.sh`: removes the worktree and deletes its local branch, refusing (**exit 2**) when the branch has commits not yet landed on the landing branch unless `--force` (which discards them). Refuses to run from inside the worktree being removed; a missing worktree/branch is a no-op, not an error.
- **`cg worktree list` — inventory worktrees.** Lists every worktree of the repository as a table (SLUG, AHEAD, BRANCH, PATH), annotating session worktrees (branch with the configured prefix) with their slug and how many commits they are ahead of the local landing ref. Network-free: AHEAD is computed against the local remote-tracking ref (`?` when unavailable). Replaces ad-hoc `git worktree list --porcelain | awk` parsing.
...
- Auto-update design + state-file reference documented in [`docs/auto-update.md`](docs/auto-update.md); README gains a "Staying up to date" section and a Go Report Card badge.

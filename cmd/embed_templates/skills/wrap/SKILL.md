...
2. Offer the removal command to the user, then run it on confirmation:
   - Use `cg worktree remove <slug>` — it removes the worktree and deletes the associated `session/<slug>` branch. It refuses in **two independent cases**: unlanded commits on the branch (exit 2), and a **dirty working tree** — modified, staged or untracked files (git refuses; exit 128). Phase 7 has already landed the commit, so the unforced path is the norm; do not reach for `--force` without asking, since it discards **both** the unlanded commits and any uncommitted files.

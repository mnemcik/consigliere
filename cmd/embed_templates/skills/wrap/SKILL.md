...
2. Offer the removal command to the user, then run it on confirmation:
   - Use `cg worktree remove <slug>` — it removes the worktree and deletes the associated `session/<slug>` branch, and refuses (exit 2) if the branch has commits not yet landed. Phase 7 has already landed the commit, so the unforced path is the norm; do not reach for `--force` without asking, since it discards the unlanded work.

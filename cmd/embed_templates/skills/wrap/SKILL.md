...
After all updates are written, commit in each repo you touched this session (the workspace, plus any external repos).
...
7. **Push.**
   - **End mode:** run `cg worktree land` from inside the worktree, with no arguments. It pushes HEAD to the landing branch (default `main`), rebasing and retrying on a non-ff rejection, and prints the landed SHA. Exit 3 means a rebase conflict was left in progress — resolve it before reporting the wrap as green; exit 4 means the push failed. (The optional argument is a commit SHA, not a slug.) For external repos that require review, fall back to the repo's default: branch-and-PR.
...
8. **Verify the commit reaches the right ref.**
   - **End mode:** `git merge-base --is-ancestor <wrap-sha> origin/main`. This assertion is a trust-boundary check between the wrap skill and whatever performed the push — tautological under the ephemeral-branch-worktree model (where push inherently carries the branch to `main`) but kept as defense-in-depth because the cost is one `merge-base` call and it catches entire classes of silent-failure: a push-command that returned 0 but actually no-op'd (e.g., refs/main pinned by a hook), a landing wrapper that crashed mid-push and left `origin/main` stale, or a user-side mistake that took the session out of the worktree before pushing. If the assertion fails, do **not** report the wrap as green; surface the divergent SHAs to the user and point at `cg worktree land` as the recovery path.
...
These rules exist because of a sequence of incidents. On 2026-04-20 an earlier version of this skill bundled four unrelated in-flight files into a wrap commit (`030c8a0`), pushing mid-work state from a parallel session straight to `main`; a follow-up the same day (`6c15665`) showed that even a correctly scoped `git add <path>` leaks parallel-session work when the index is shared — the fix was per-session worktrees plus the pre-commit staged-set check (step 5). Between 2026-04-22 and 2026-04-23, three further failures (`c034b55`, `0590d30`, `5593a6a`) hit the detached-HEAD worktree model: wrap commits advanced only the detached HEAD and never fast-forwarded `main`, and cherry-pick-only landing scripts silently orphaned chain ancestors; step 8 and the landing script were the incremental mitigations. On 2026-04-24 the detached-HEAD model was replaced with ephemeral `session/<slug>` branches per worktree, making orphans structurally impossible and shrinking the landing logic accordingly — the model now shipped as `cg worktree create` / `land` / `remove`. The authoritative current rules live in the workspace's `CLAUDE.md` → Git Workflow → "Commit Scope — This Session Only", "Worktrees for Parallel Sessions", and "External Repos — Parallel Sessions".
...
2. Offer the removal command to the user, then run it on confirmation:
   - Use `cg worktree remove <slug>` — it removes the worktree and deletes the associated `session/<slug>` branch. It refuses (exit 2) in **two independent cases**: unlanded commits on the branch, and a **dirty working tree** — modified, staged or untracked files. Both list what they found. Phase 7 has already landed the commit, so the unforced path is the norm; do not reach for `--force` without asking, since it discards **both** the unlanded commits and any uncommitted files.
   - **Do not verify landedness and stop there.** `git merge-base --is-ancestor` proves only that *committed* work reached the landing branch; it says nothing about uncommitted or untracked files still sitting in the directory. Removal is only lossless when **both** conditions hold, and only the unforced command checks the second. An unforced `cg worktree remove` exiting 0 is therefore positive evidence the worktree was clean — treat that exit code as the proof, and never substitute `--force` for it when a removal is unexpectedly refused. (That evidence is cg's to give: plain `git worktree remove` honours `status.showUntrackedFiles`, so under `=no` it deletes an untracked-only worktree silently with exit 0. cg checks untracked files regardless of config.)
   - For a non-session worktree (one `cg` did not create), fall back to `git worktree remove <path>` followed by the appropriate branch cleanup (e.g., `git branch -d <branch>`).
...
For each external repo touched this session (not the workspace itself, which commits directly to `main`):
...
- **Pause mode:**
  ...
  ```
  ⏸️ Session paused — resume by re-entering the project (worktree at <path>).
  ```
